 ...
 */
export const PermissionGuard = ({ permissions, children, fallback = null, requireAll = false }) => {
	const { hasPermission } = useRouteAccess();
	if (!permissions) return children;
	const list = Array.isArray(permissions) ? permissions : [permissions];
	const hasAccess = requireAll ? list.every((m) => hasPermission(m)) : list.some((m) => hasPermission(m));
